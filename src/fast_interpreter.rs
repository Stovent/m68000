...
impl M68000 {
    ...
    pub(super) fn fast_unknown_instruction(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
        Err(Vector::IllegalInstruction as u8)
    }

    pub(super) fn fast_abcd(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_add(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_adda(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_addi(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_addq(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_addx(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_and(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, dir, size, am) = self.register_direction_size_effective_address(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_andi(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (size, am, imm) = self.size_effective_address_immediate(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_andiccr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_andisr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        if self.regs.sr.s {
            let imm = self.immediate(memory);
            self.regs.sr &= imm;
            Ok(EXEC::ANDISR)
    ...
    pub(super) fn fast_asm(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_asr(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_bcc(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let pc = self.regs.pc;
    ...
    pub(super) fn fast_bchg(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (am, mut count) = self.effective_address_count(memory);
    ...
    pub(super) fn fast_bclr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (am, mut count) = self.effective_address_count(memory);
    ...
    pub(super) fn fast_bra(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let pc = self.regs.pc;
    ...
    pub(super) fn fast_bset(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (am, mut count) = self.effective_address_count(memory);
    ...
    pub(super) fn fast_bsr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let pc = self.regs.pc;
        let disp = self.displacement(memory);
    ...
    pub(super) fn fast_btst(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (am, mut count) = self.effective_address_count(memory);
    ...
    pub(super) fn fast_chk(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_clr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_cmp(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_cmpa(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, size, am) = self.register_size_effective_address(memory);
    ...
    pub(super) fn fast_cmpi(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (size, am, imm) = self.size_effective_address_immediate(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_cmpm(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_dbcc(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let pc = self.regs.pc;
    ...
    /// process_exception method returns the exception processing time.
    pub(super) fn fast_divs(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, am) = self.register_effective_address(memory);
    ...
    /// If a zero divide exception occurs, this method returns the effective address calculation time, and the
    /// process_exception method returns the exception processing time.
    pub(super) fn fast_divu(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, am) = self.register_effective_address(memory);
    ...
    pub(super) fn fast_eor(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, _, size, am) = self.register_direction_size_effective_address(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_eori(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (size, am, imm) = self.size_effective_address_immediate(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_eoriccr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let imm = self.immediate(memory);
    ...
    pub(super) fn fast_eorisr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        if self.regs.sr.s {
            let imm = self.immediate(memory);
            self.regs.sr ^= imm;
            Ok(EXEC::EORISR)
    ...
    pub(super) fn fast_exg(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_ext(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_illegal(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
        Err(Vector::IllegalInstruction as u8)
    }

    pub(super) fn fast_jmp(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_jsr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let am = self.effective_address(memory);
    ...
    pub(super) fn fast_lea(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, am) = self.register_effective_address(memory);
    ...
    pub(super) fn fast_link(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_lsm(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (dir, am) = self.direction_effective_address(memory);
    ...
    pub(super) fn fast_lsr(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
        let (rot, dir, size, mode, reg) = self.rotation_direction_size_mode_register();
    ...
    pub(super) fn fast_move(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_movea(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_moveccr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let am = self.effective_address(memory);
    ...
    pub(super) fn fast_movefsr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let am = self.effective_address(memory);
    ...
    pub(super) fn fast_movesr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        if self.regs.sr.s {
            let am = self.effective_address(memory);
            let mut ea = EffectiveAddress::new(am, Some(Size::Word));
    ...
    pub(super) fn fast_moveusp(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
        if self.regs.sr.s {
            let (d, reg) = self.direction_register();
    ...
    pub(super) fn fast_movem(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (dir, size, am, mut list) = self.direction_size_effective_address_list(memory);
    ...
    pub(super) fn fast_movep(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_moveq(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_muls(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, am) = self.register_effective_address(memory);
    ...
    pub(super) fn fast_mulu(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, am) = self.register_effective_address(memory);
    ...
    pub(super) fn fast_nbcd(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let am = self.effective_address(memory);
    ...
    pub(super) fn fast_neg(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (size, am) = self.size_effective_address(memory);
    ...
    pub(super) fn fast_negx(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (size, am) = self.size_effective_address(memory);
    ...
    pub(super) fn fast_nop(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
        Ok(EXEC::NOP)
    }

    pub(super) fn fast_not(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (size, am) = self.size_effective_address(memory);
    ...
    pub(super) fn fast_or(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, dir, size, am) = self.register_direction_size_effective_address(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_ori(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (size, am, imm) = self.size_effective_address_immediate(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_oriccr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let imm = self.immediate(memory);
    ...
    pub(super) fn fast_orisr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        if self.regs.sr.s {
            let imm = self.immediate(memory);
            self.regs.sr |= imm;
            Ok(EXEC::ORISR)
    ...
    pub(super) fn fast_pea(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let am = self.effective_address(memory);
    ...
    pub(super) fn fast_reset(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        if self.regs.sr.s {
    ...
    pub(super) fn fast_rom(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (dir, am) = self.direction_effective_address(memory);
    ...
    pub(super) fn fast_ror(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
        let (rot, dir, size, mode, reg) = self.rotation_direction_size_mode_register();
    ...
    pub(super) fn fast_roxm(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (dir, am) = self.direction_effective_address(memory);
    ...
    pub(super) fn fast_roxr(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
        let (rot, dir, size, mode, reg) = self.rotation_direction_size_mode_register();
    ...
    pub(super) fn fast_rte(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        if self.regs.sr.s {
    ...
    pub(super) fn fast_rtr(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_rts(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        self.regs.pc = self.pop_long(memory)?;

        Ok(EXEC::RTS)
    }

    pub(super) fn fast_sbcd(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_scc(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_stop(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let imm = self.immediate(memory);
    ...
    pub(super) fn fast_sub(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, dir, size, am) = self.register_direction_size_effective_address(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_suba(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (reg, size, am) = self.register_size_effective_address(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_subi(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (size, am, imm) = self.size_effective_address_immediate(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_subq(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (imm, size, am) = self.data_size_effective_address(memory);
        let mut exec_time;
    ...
    pub(super) fn fast_subx(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_swap(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
    ...
    pub(super) fn fast_tas(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let am = self.effective_address(memory);
    ...
    pub(super) fn fast_trap(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
        let vector = self.vector();
        Err(Vector::Trap0Instruction as u8 + vector)
    }

    pub(super) fn fast_trapv(&mut self, _: &mut impl MemoryAccess) -> InterpreterResult {
        if self.regs.sr.v {
    ...
    pub(super) fn fast_tst(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let (size, am) = self.size_effective_address(memory);
    ...
    pub(super) fn fast_unlk(&mut self, memory: &mut impl MemoryAccess) -> InterpreterResult {
        let reg = self.register();
